...
describe EntriesController do
  ...
  describe "#show" do
    it "is successful" do
      get :show, id: "#{yesterday.strftime("%m-%d-%Y")}"
      expect(response).to be_success
    end
  end
  ...
  describe "#create" do
    context "when a prior person is not found" do
      it "creates a Person for the email/name if one does not exist" do
        attrs = { person_name: "test", person_email: "test@example.com", entry: { yesterday: "worked on yesterday", today: "worked on today" } }
    ...
    it "displays the error page if the new Entry  does not validate" do
      attrs = { person_name: "test", person_email: "test@example.com", entry: { yesterday: "" } }
      post :create, attrs
      expect(response).to render_template(:new)
    end
    ...
    it "adds the entry to the Person" do
      ...
      attrs = { person_name: person.name, person_email: person.email, entry: { yesterday: "worked on yesterday", today: "worked on today" } }
